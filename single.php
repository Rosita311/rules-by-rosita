  ...
  <section class="single-post__hero hero-section">
    <div class="hero-image">
      <?php if ( has_post_thumbnail() ):
        ...
        if ( ! $alt_text ) $alt_text = get_the_title();
      ?>
        <img
      ...
      <article class="single-post__content-wrapper" <?php post_class(); ?>>
          ...
          <nav aria-label="Berichtinformatie">
            <ul class="post-meta__list">
              ...
              <li class="post-meta__item">
                ...
                <span class="sr-only">Gepubliceerd op: <?php the_time( 'j F Y' ); ?></span>
                <time datetime="<?php the_time( 'Y-m-d' ); ?>" aria-hidden="true"><?php the_time( 'j-m-Y' ); ?></time>
              </li>

              <li class="post-meta__item">
                <?php echo rules_by_rosita_icon( 'messages' ); ?>
                <?php comments_number( '0', '1', '%' ); ?>
                <span class="sr-only">reacties</span>
              </li>

              <li class="post-meta__item">
                <?php echo rules_by_rosita_icon( 'clock' ); ?>
                <span class="sr-only">Leestijd:</span>
                <?php echo rules_by_rosita_get_reading_time(); ?>
              </li>
        ...
        <nav class="post-nav" aria-label="Post navigatie">
          <div class="post-nav__prev">
            <?php if ( $prev_post ): ?>
              <a class="btn btn__icon--large" href="<?php echo esc_url( get_permalink( $prev_post ) ); ?>">
          ...
          <div class="post-nav__next">
            <?php if ( $next_post ): ?>
              <a class="btn btn__icon--large" href="<?php echo esc_url( get_permalink( $next_post ) ); ?>">
        ...
        if ( $related_query->have_posts() ): ?>
          <ul class="blog-listing__grid">
            <?php while ( $related_query->have_posts() ) : $related_query->the_post();
              get_template_part( 'template-parts/' . ( has_post_thumbnail() ? 'card' : 'card-fallback' ) );
            endwhile;
            wp_reset_postdata(); ?>
          </ul>
        <?php else: ?>
